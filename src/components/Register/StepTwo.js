...
function StepTwoRegister({ setSuccessCode, handleNext, email }) {
  ...
  const verifyHandler = async (code) => {
    console.log(email)
    const verify = await verifyCode(parseInt(code), email)
  ...
  return (
    <div>
      ...
      <p className='text-muted text-sm'>Didn't get an email? Send it again here</p>
    </div>
  );
}
